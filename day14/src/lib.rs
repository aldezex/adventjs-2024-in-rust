...
pub fn min_moves_to_stables(reindeer: Vec<i32>, stables: Vec<i32>) -> i32 {
    let mut reindeer = reindeer;
    let mut stables = stables;
    reindeer.sort();
    stables.sort();
    let mut moves = 0;
    for i in 0..reindeer.len() {
        moves += (reindeer[i] - stables[i]).abs();
    }
    moves
}
...
mod tests {
    ...
    #[test]
    fn test_min_moves_to_stables() {
        assert_eq!(min_moves_to_stables(vec![2, 6, 9], vec![3, 8, 5]), 3);
        assert_eq!(min_moves_to_stables(vec![1, 1, 3], vec![1, 8, 4]), 8);
    }
}
